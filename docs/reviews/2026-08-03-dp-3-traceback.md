...
정본 가이드에 명시한 「원문을 본문에서 드러내지 않는다」를 적용했다. 이 리포트가 지적한 사항이 아니다.

- `:11` "이 글은 강의 노트 밖 확장으로, 채운 표에 분할점을 함께 적어…" → "이 글은 채운 표에 분할점을 함께 적어…". 확장 경계를 밝히던 구절만 걷었고 글이 다루는 질문은 그대로 남았다.

위 🟢 [L6] finding은 "본문 11행도 '이 글은 강의 노트 밖 확장으로'라고 스스로 범위를 밝혀 독자가 원문 밖임을 알 수 있다"를 판정 근거의 하나로 삼았다. 그 문장이 사라졌으므로 **L6(원문 충실성)는 재리뷰 대상이다.** 본문이 스스로 범위를 밝히지 않게 된 뒤에도 추가 내용을 원문의 것으로 오인할 여지가 없는지 판정이 필요하다. 확장 경계와 provenance 기록은 이 리포트와 포스트별 스펙에 남아 있다.
