...
🟡 3건을 모두 반영했다. 판정은 2026-08-05 시점 근거로 그대로 남긴다.

- 🟡 [D12] 시리즈 다음 편 링크 → **반영 완료**. "이어지는 글"의 Plane Sweeping 문장에 `/blog/closest-pair-3` 링크를 붙였다. 결정적 검사 재실행에서 D12 미발생.
- 🟡 [L6] :137 원문의 메모리 대가 → **반영 완료**. 밴드를 별도 배열에 담으면 호출마다 할당이 생긴다는 단서를 코드 앞 단락에 적고, 강의 노트가 그 대가를 지적하며 건너뛰는 구현을 택했다는 사실과 차수는 같고 상수 인자만 다르다는 점을 함께 밝혔다.
- 🟡 [L7] :215 하한 인용의 모형 → **반영 완료**. 하한 문장을 증명 블록에서 빼내 비고로 옮겼다. 1편의 element uniqueness 환원을 가리키고, 2차원이 1차원을 특수한 경우로 포함해 같은 하한을 물려받는다는 단계를 적었다. "그 모형 안에서 최적"으로 범위를 한정하고 모형을 벗어난 계산에는 적용되지 않음을 덧붙였다. 핵심 정리 항목도 같은 표현으로 맞췄다.

반영 후 검증: 결정적 검사 재실행 "발견 사항 없음 ✅", `npm run build` 성공. 코드와 복잡도 유도는 바꾸지 않았다.
