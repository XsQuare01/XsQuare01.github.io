...
### 🟢 [L4] src/content/posts/floyd-warshall-memory.md:54

- severity: 🟢
- source: L
- rule_id: L4
- location: src/content/posts/floyd-warshall-memory.md:54
- quote: ![N³칸을 두 층으로, 다시 한 층으로 줄인다.](/images/floyd-warshall-memory/layers.svg)
- message: 검토 완료, 이슈 없음. layers.svg를 텍스트와 렌더링으로 확인했으며 N³→2N²→N², prev/curr, D[k%2], 진행 화살표가 본문과 일치하고 클리핑이 없다. overwrite-safe.svg도 $k$행·$k$열 강조, $(k,k)=0$, 열 방향 등식과 덮어쓰기 결론이 본문 83–112행과 일치하며 경로·레이블·강조의 잘림이 없다. 행 방향 등식은 그림에서 대칭으로 암시되고 본문에서 명시된다.
...
### 🟢 [L7] src/content/posts/floyd-warshall-memory.md:42

- severity: 🟢
- source: L
- rule_id: L7
- location: src/content/posts/floyd-warshall-memory.md:42
...
- 본편 `all-pairs-shortest-path.md:189`의 🔴 [L7]을 고치며 본편 의사코드에 `INF` 검사가 들어갔다. 이 글의 두 의사코드(2층 :34–, 1층 :62–)에는 검사를 넣지 않고, 검사를 덜어냈다는 사실과 그 규칙이 층 수와 무관하게 그대로 필요하다는 점을 :32에 한 문장으로 밝혔다. 이 글의 주제가 메모리 축소여서 층 인덱스가 보여야 하고, 1층 코드의 결론인 "세 겹 `for`와 `min` 하나"라는 축소 논지를 검사 상용구가 덮지 않게 하기 위해서다.
