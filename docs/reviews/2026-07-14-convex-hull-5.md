...
## 추가 반영 (사용자 지적, 2026-07-14)

- 지적: `antipodal.svg`의 대척점 선분과 지지선이 수직이 아닌데, 본문은 "지름에 수직인 평행선이 두 끝점에 닿는다"고 서술 → 도판(일반 대척점 쌍)과 문장(지름의 수직 특수성)이 나란히 놓여 "대척점=수직" 오해를 유발.
- **반영 완료**: `antipodal.svg`를 **지름 쌍 (120,190)-(560,300)에 수직인**(기울기 −4) 두 평행 지지선 구성으로 다시 그림. 두 지지선이 껍질을 정확히 사이에 둠을 검산(`y+4x`가 a=670=min, b=2540=max, 나머지 그 사이) → 접선으로 타당. 본문에서 이 도판을 **수직 논증 문단 뒤**로 옮기고, 대척점 정의에 "선분은 지지선과 수직일 필요가 없다"를 명시해 일반/특수(지름)를 구분. 캡션도 갱신.
