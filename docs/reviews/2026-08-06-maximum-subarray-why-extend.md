...
- 「이어 붙이기가 최선인 이유」의 증명이 읽기 어렵다는 지적 → 인덱스 $s$ 로 경우를 나누던 서술을 걷어내고 "짝짓기 → 짝의 합 비교 → 최댓값 옮기기" 세 단계로 나눠 다시 썼다. $S_i$·$T_{i+1}$ 정의도 산문 대신 두 줄 목록으로 바꿨다. 논증의 구조와 결론은 그대로다.
- 증명 끝의 $\blacksquare$ 기호 삭제 → 이 글의 두 증명에서 모두 뺐다. 같은 기호를 쓰는 `scheduling-1.md` 는 이번 요청 범위 밖이라 손대지 않았다.
