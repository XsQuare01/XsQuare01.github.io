...
- 최대 8개 결과 표시
- 빈 쿼리면 결과 초기화
- 각 결과: `<a href="/blog/${item.id}">` 클릭 시 페이지 이동
  - `post.id`는 Astro Content Collections에서 파일명 기반 slug (예: `"rsa"`) → URL `/blog/rsa`와 일치
...
- `#search-modal`: `position:fixed; inset:0; z-index:1001; background:rgba(0,0,0,0.6)`
  - 기존 요소 z-index: `#back-to-top:200`, 읽기 진행 바:1000 → 모달은 1001로 모든 요소 위에 위치
...
- 플랫폼별 단축키 표시 분기 — 버튼 텍스트는 `Ctrl K`로 통일, JS에서는 둘 다 처리
