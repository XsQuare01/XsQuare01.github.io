...
바깥 루프는 정렬 순서를 따라 각 정점을 정확히 한 번 현재 정점으로 삼고, 현재 정점은 pop 루프를 끝낸 뒤 조건 없이 한 번 push된다. 한 정점이 두 번 현재 정점이 되는 일은 없으므로 push 횟수는 정점마다 정확히 한 번이다. pop된 정점이 다시 push되지 않는 근거도 이 단일 순회 구조다. pop은 그 정점이 껍질에 남는지를 판정할 뿐, 다시 push될 수 있는지를 정하지 않는다. pop 횟수는 push 횟수를 넘지 못한다. push와 pop을 각각 1 연산으로 세면 스택 연산의 총 횟수는 $2n$ 이하이므로 $O(n)$이다. 앞선 정렬 비용 $O(n\log n)$은 이 주장에 포함되지 않는다.

**개선 이유:**

「정확히 한 번」이 만든 두 의무를 각각 닫았다. 존재성은 현재 정점마다 조건 없이 일어나는 push로, 중복 배제는 한 정점이 두 번 현재 정점이 되지 않는 단일 순회 구조로 닫힌다. pop의 의미를 중복 배제의 근거로 쓰지 않은 것이 요점이다. 무엇을 1 연산으로 세는지 비용 모델을 밝히고 정렬 비용을 분리해 주장의 적용 영역을 좁혔다. 논증의 타당성은 별도 검증 자료를 바탕으로 리뷰에서 확정한다.
```
