...
**정리 1**(`<div class="thm">` + `<div class="thm-head">정리 1<span class="en">중복 노드 없는 길로 바꾸기</span></div>`): 가중치가 비음수이면, 중간에 거칠 수 있는 노드를 집합 $S$로 제한할 때 그 제한을 지키는 $i \to j$ 길은 어느 것이든 같은 노드를 두 번 지나지 않는 길로 바꿀 수 있다. 바꾼 길도 같은 제한을 지키고 길이는 늘지 않는다.

최단이라는 조건을 정리의 전제에 넣지 않는다. 최솟값이 달성된다는 사실을 이 정리로 세울 것이므로, 전제에 최단을 두면 순환이 된다.

**증명**(`<div class="proof">` + `<span class="proof-lead">증명.</span>` … `<span class="qed">∎</span>`): 제한을 지키는 길 $Q$에서 어떤 노드 $a$가 두 번 나오면 그 사이 구간은 $a$에서 출발해 $a$로 돌아오는 닫힌 구간이다. 이 구간을 잘라내도 양 끝이 $a$로 이어지므로 여전히 $i \to j$ 길이고, 중간 노드가 줄어들 뿐이라 제한도 그대로 지킨다. 잘라낸 구간의 길이는 비음수이므로 전체 길이는 늘지 않는다. 같은 노드가 남아 있으면 반복한다. 한 번 잘라낼 때마다 노드 등장 횟수가 최소 하나 줄어드니 유한 번에 멈춘다. **비음수 전제가 여기서 쓰인다**는 점을 명시.

- [ ] **Step 8: `### 경로 하나를 골라 가른다` — 스펙 §4 B**

길이 하나도 없어 세 값이 모두 $\infty$인 경우를 먼저 털어 둔다. 그다음 최솟값이 실제로 달성된다는 것을 정리 1로 확인한다(중복 노드 없는 길은 유한 개, 정리 1이 아무 길이나 그 안으로 보낸다).

최단 경로가 여러 개일 수 있다. "그 최단 경로"라고 지목하는 대신 **아무거나 하나를 골라** $P$라 한다. 정리 1로 $P$를 같은 노드를 두 번 지나지 않는 길로 바꿔도 최단이다.
...
**$k$를 지나는 경우.** $P$가 같은 노드를 두 번 지나지 않으므로 $k$는 정확히 한 번 나온다. 그 자리에서 $P$를 $P_1: i \to k$와 $P_2: k \to j$로 자른다. 두 조각의 중간 노드는 $P$의 중간 노드에서 $k$를 뺀 것이므로 모두 $\{1,\dots,k-1\}$ 안에 있다. 각각 $D^{k-1}[i][k]$, $D^{k-1}[k][j]$의 후보이므로 $D^{k-1}[i][k] + D^{k-1}[k][j] \le |P_1| + |P_2| = |P|$.
...
`### 눈금으로 돌아오면` — Step 4에서 세운 $O(M\log N)$~$O(NM\log N)$ 눈금을 회수한다. 성긴 그래프($M \simeq N$)에서 다익스트라 $N$번은 $O(N^2\log N)$이므로 $O(N^3)$은 눈금 위쪽 끝을 넘는다. 즉 **성긴 그래프에서는 진다**. 밀집 그래프($M \simeq N^2$)에서는 눈금 안에 들어오며, 그쪽 비교는 후속편으로 넘긴다(구현이 갈리므로 한 줄로 끝나지 않음).
...
*정의로 보는 논증.* 두 값의 차이는 중간에 $k$를 쓸 수 있느냐뿐이다. $i \to k$로 가는 길에서 $k$는 끝점이다. 본편 정리 1이 아무 길이나 같은 노드를 두 번 지나지 않는 길로 바꿔 주므로 두 제한 어느 쪽에서든 그런 길만 겨루어도 최솟값은 그대로이고, 그런 길에서 $k$는 끝점으로 이미 나왔으니 중간 노드로 쓰일 수 없다. 허용 여부가 결과를 바꾸지 못하므로 두 값은 같다.
...
비교 대상을 명시한다. 밀집 그래프($M \simeq N^2$)에서 다익스트라는 구현이 둘로 갈린다. 이진 힙이면 $N$번에 $O(N^3\log N)$, 배열 argmin이면 $N$번에 $O(N^3)$이다. [다익스트라 2](/blog/dijkstra-2)가 이 구간에서 배열 argmin을 권하므로 그쪽이 실제 비교 상대다. 플로이드·워셜도 $O(N^3)$이라 **표기는 비긴다**.

앞 글과 어긋나므로 "$\log N$ 하나 차이"로 쓰지 않는다. 표기가 갈라 주지 못하니 갈림은 상수와 메모리 접근 패턴에 있다고 쓴다. 플로이드·워셜은 우선순위 큐도 시작점마다의 초기화도 없이 배열 세 겹 반복만 돌고 메모리 접근이 규칙적이다.

**단정하지 말 것**(스펙 §8): 측정 자료가 없으므로 "실측이 몇 배 빠르다"고 쓰지 않는다. 반대로 플로이드·워셜이 표기에서 이긴다고도 쓰지 않는다. 이유를 밝히는 선에서 멈춘다.
