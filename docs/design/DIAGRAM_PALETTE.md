...
- 검사 3은 파일 텍스트 전체를 훑는다. 주석 안에 남긴 색도 위반으로 잡히므로 쓰지 않는 색은 지운다.
- 단계 도식은 `public/` 아래에 두고 파일명을 `-steps.svg`로 끝낸다. 플러그인이 그 규약으로 대상을 찾고 검사 3도 같은 규약으로 훑는다.
- 색을 하나 더 들이는 일은 다섯 곳을 고치는 일이다 — `test/palette.test.mjs`의 이름 목록, `global.css`의 토큰, 같은 파일의 `fill`·`stroke` 매핑 두 줄, 이 문서의 코드 블록, 이 문서의 표.
- 도식의 구조 규약(단계 그룹은 `<svg>`의 최상위 자식, 공통 내용은 그룹보다 앞, 그룹 안에 `<g>` 중첩 금지)은 `src/plugins/rehype-svg-steps.mjs`의 주석에 있다. 어기면 검사가 아니라 조용한 `<img>` 강등으로 나타난다.

## 레거시 8색 — 문서에 남기고 파일은 건드리지 않는다

| 지금 | 파일 수 (2026-08-21) | 대체 | 이유 |
| --- | --- | --- | --- |
| `#f97316` | 61파일 | `#fbbf24` | `DESIGN_DIRECTION.md` §4가 이미 걷어내라고 적었다. 대비도 6.7 → 11.3으로 오르고, 오렌지는 빨강과 인접해 「주의」와 「거부」가 붙어 보인다. |
| `#34d399` | 42파일 | `#4ade80` | 같은 역할의 다른 계열(emerald vs green). 채택률이 두 배 높은 쪽을 남긴다. |
| `#22c55e` | 34파일 | `#4ade80` | 같은 계열의 어두운 단계(8.3:1). 두 값을 나눠 쓴 적이 없다. |
| `#ef4444` | 26파일 | `#f87171` | 빨강 두 단계를 구별해 쓴 적이 없다. 대비가 높은 쪽을 남긴다(6.8 대 5.0). |
| `#4a5568` | 29파일 | `#475569` | Tailwind v1 회색 잔재. v3 슬레이트로 통일한다. |
| `#2d3748` | 29파일 | `#1e293b` | 같은 이유. 대비 1.6:1로 면 역할이 겹친다. |
| `#1f2937` | 8파일 | `#1e293b` | 또 다른 회색 계열(gray-800). |
