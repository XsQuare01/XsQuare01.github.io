...
기준선은 언제나 지금의 두 점 $a$, $b$를 지나는 직선 $ab$ 하나다.

- 오른쪽 껍질에서 $b$의 아래쪽 이웃이 직선 $ab$보다 아래에 있으면, 직선이 아직 오른쪽 껍질을 파고든다. $b$를 그 이웃으로 내린다.
- 왼쪽 껍질에서 $a$의 아래쪽 이웃이 같은 직선 $ab$보다 아래에 있으면, $a$를 그 이웃으로 내린다.
- 양쪽 다 더 내릴 데가 없으면, 곧 어느 껍질에도 직선 아래로 삐져나온 점이 없으면 그게 하단 공통 접선이다.
