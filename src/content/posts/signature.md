...
<ul>
<li>RSA 서명은 복호화 키($d$)로 서명하고 암호화 키($e$)로 검증한다. $ed \equiv 1 \pmod{\varphi(n)}$에서 $m^{ed} = m$이 성립하기 때문이다.</li>
<li>ElGamal 서명은 DLP에 기반하며, $g^m \equiv y^r \cdot r^s \pmod{p}$로 검증한다. 비밀키 $a$ 없이는 유효한 $(r, s)$를 만들 수 없다.</li>
<li>SHA로 $m \to h$(고정 크기)를 만들어 서명하면 서명 크기가 메시지 크기와 무관해진다. 충돌 저항성이 Not Convertible을 보장한다.</li>
