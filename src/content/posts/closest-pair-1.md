...
그렇지 않다. 1차원 가장 가까운 쌍을 풀면 **element uniqueness**, 즉 "주어진 $n$개 값이 모두 서로 다른가"를 곧바로 답할 수 있다. 가장 가까운 두 값의 거리가 0인지 보면 되기 때문이다. 그리고 element uniqueness는 **대수적 결정 트리**(algebraic decision tree) 모형에서 $\Omega(n \log n)$이 하한임이 알려져 있다. 1차원 가장 가까운 쌍은 그 문제를 풀어 주므로 같은 모형에서 같은 하한을 갖는다. 정렬 후 인접 쌍 비교가 이 하한에 닿으니 최적이다.
...
합치기 단계에서 분할선을 가로지르는 쌍 중 현재 최솟거리 $D$보다 짧을 가능성이 있는 쌍은 어디에 있는가? 답은 **밴드** 안이다.

<div class="thm def">
<div class="thm-head">정의 1<span class="en">band</span></div>

현재 최솟거리 $D = \min(D_L, D_R)$에 대해, 분할선에서 x좌표 차이가 $D$ 이내인 점들의 모음을 **밴드(band)** 라 한다.

</div>

분할선에서 x좌표 차이가 $D$를 넘는 쌍은 x좌표 차이만으로도 이미 거리가 $D$ 이상이라 $D$를 갱신할 수 없다. 따라서 밴드 안 점들만 반대편의 밴드 점과 비교하면 된다.
...
<div class="thm">
<div class="thm-head">보조정리 1<span class="en">상수 개 비교로 충분</span></div>
...
$D$는 왼쪽과 오른쪽 각 영역의 최솟거리다. 같은 편 두 점은 항상 거리 $\ge D$다. $P$ 위쪽으로 y 차이가 $D$ 미만인 범위는 폭 $2D$(분할선 좌우 각 $D$) × 높이 $D$인 직사각형이다.

이 직사각형을 분할선 기준으로 왼쪽 $D 	imes D$와 오른쪽 $D 	imes D$로 가른다. 한 변 $D$인 정사각형에는 같은 편 점이 최대 4개까지만 들어간다. 한 변 $D/2$인 칸 4개로 다시 쪼개면 칸의 대각선이 $D$보다 짧아 칸마다 같은 편 점이 하나뿐이기 때문이다. 양쪽을 합치면 $P$ 포함 최대 8점 → 다음 **7개**면 충분하다.

칸이 아니라 편으로 먼저 가르는 이유가 있다. x좌표가 같은 점들은 분할선 위에 놓여 양쪽 재귀로 갈릴 수 있고, 그러면 한 칸 안의 두 점이 같은 편이라는 보장이 사라진다. 자세한 논거는 [별도 글](/blog/closest-pair-five-points)에서 다룬다.
...
제곱근 계산은 부동소수점 오차를 유발할 수 있다. 두 거리를 비교할 때 $d_1 < d_2 \iff d_1^2 < d_2^2$ (두 값 모두 양수일 때)이므로, 제곱 상태로 저장하고 비교한다. 코드 전체에서 거리는 **제곱값**이며, 밴드 폭 비교도 x좌표 차이를 제곱해 $d$(거리 제곱)와 비교한다.

`long long`이 안전한 것은 **좌표 범위가 정해져 있을 때**다. 좌표의 절댓값이 $C$ 이하이면 $|dx|, |dy| \le 2C$이므로 제곱거리는 최대 $8C^2$다. $C = 10^9$이면 $8 \times 10^{18}$로 `long long` 상한($\approx 9.2 \times 10^{18}$) 안에 겨우 들어간다. 범위를 모르는 입력이라면 곱셈 단계에서 이미 넘칠 수 있으므로 거리 제곱을 `__int128`로 계산한다.
...
분할 정복으로 가장 가까운 점 쌍을 구하는 알고리즘의 전체 시간 복잡도는 $O(n \log^2 n)$이다.
...
- 거리는 **제곱값**으로 다뤄 부동소수점 루트 계산을 피한다. 좌표 절댓값이 $10^9$ 이하면 `long long`으로 충분하고, 범위를 모르는 입력이면 중간 계산에 `__int128`이 필요하다.
