...
- **입력 변경.** `edges.sort(...)`는 입력 리스트를 in-place로 변경한다. 원본을 보존해야 한다면 `sorted_edges = sorted(edges, key=lambda e: e[2])`로 분리하는 편이 안전하다.
- **연결성 가정.** 그래프가 연결되지 않은 경우 종료 시 `len(mst) < n - 1`이고, 반환되는 것은 엄밀히 말해 **최소 신장 포레스트(MSF)** 다. 호출 측에서 길이 검사로 연결성 여부를 판단할 수 있다.
