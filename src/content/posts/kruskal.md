...
description: "Kruskal 알고리즘으로 최소 신장 트리(MST)를 구성하는 과정을 따라간다. 가장 작은 엣지부터 그리디하게 선택하는 단순한 전략이 왜 최적해를 보장하는지 루프 불변식으로 증명하고, Union-Find로 사이클 검사를 거의 상수 시간에 처리해 전체 O(m log m)으로 마무리한다."
...
<li>루프 불변식으로 정확성 증명 — 가중치 비교의 세 경우 분석</li>
...
Kruskal이 정말로 MST를 찾는다는 것을 **루프 불변식**으로 증명한다.

**불변식:** 알고리즘이 현재까지 추가한 엣지 집합 $T$는 어떤 MST $T_{mst}$의 부분집합이다.

$$T \subseteq T_{mst}$$

이것이 매 반복마다 유지된다는 것만 보이면, 종료 시 $|T| = n - 1$이고 신장 트리의 크기 역시 $n - 1$이므로 $T = T_{mst}$, 즉 MST가 완성됨이 따라온다.

### Base case

$|T| = 0$이면 $T = \emptyset$이므로 모든 $T_{mst}$의 부분집합. 자명.

### Inductive step

$|T| = k$에서 $T \subseteq T_{mst}$가 성립한다고 가정하자. 알고리즘이 다음 엣지 $e$를 추가해 $T' = T \cup \{e\}$가 됐을 때 $T' \subseteq T_{mst}'$ ($T_{mst}'$는 같은 MST이거나 다른 MST)도 성립함을 보인다.

만약 $e \in T_{mst}$라면 $T' \subseteq T_{mst}$, 끝.

만약 $e \notin T_{mst}$라면, $T_{mst} \cup \{e\}$ 안에 $e$를 포함하는 사이클이 존재한다. 이 사이클에는 $T$에 속하지 않는 어떤 엣지 $e'$가 반드시 하나 이상 존재한다. 만약 사이클의 모든 엣지가 $T$에 있었다면 $T$가 이미 사이클을 갖는다는 뜻인데, 알고리즘이 그것을 막아왔으므로 모순이다.

이제 $w(e)$와 $w(e')$를 비교하는 세 경우를 본다.

**Case 1. $w(e) < w(e')$**

$T_{mst}' = (T_{mst} - \{e'\}) \cup \{e\}$는 여전히 신장 트리이면서 가중치 합이 $T_{mst}$보다 작다. 이는 $T_{mst}$가 MST라는 가정에 모순. **발생 불가.**

**Case 2. $w(e) > w(e')$**

정렬 순서에서 $e'$가 $e$보다 먼저 등장한다. 알고리즘이 $e'$를 봤을 때 두 경우뿐이다.

- $e'$가 사이클을 만들지 않았다 → 알고리즘은 $e'$를 추가했을 것 → $e' \in T$. 그런데 우리는 $e' \notin T$를 가정했으므로 모순.
- $e'$가 사이클을 만들었다 → $e'$의 양 끝점이 이미 같은 컴포넌트. 엣지는 추가만 되고 제거되지 않으므로, 이후 시점인 $e$ 추가 시점에도 $e'$ 양 끝점은 여전히 같은 컴포넌트. 즉 $e$를 닫는 사이클 안에 $e'$가 있다는 사실과, $e$ 추가 시점에 사이클이 없다는 알고리즘의 동작이 충돌. 모순.

두 경우 다 모순이므로 **발생 불가.**

**Case 3. $w(e) = w(e')$**

$T_{mst}' = (T_{mst} - \{e'\}) \cup \{e\}$도 같은 총 가중치를 갖는 또 다른 MST다. $T \cup \{e\} \subseteq T_{mst}'$이 성립하므로 불변식은 (다른 MST로 옮겨가서) 유지된다.

세 경우를 모두 다뤘다. 어느 경우든 불변식이 깨지지 않는다. $\square$

이 논증은 **교환 논법(exchange argument)** 의 전형이다. 알고리즘이 고른 엣지와 정답 MST의 엣지를 한 쌍씩 맞교환하면서 "더 나쁘지 않다"는 사실을 반복적으로 보이는 방식.
...
Kruskal은 그리디가 항상 손해 보지 않는다는 것을 보여주는 좋은 예다. "지금 가장 작은 엣지를 고른다"는 단순한 규칙이, 사이클을 만들지만 않는다면 반드시 어떤 MST의 일부가 된다.
