...
1. **경로 문제**: 그래프 $G$에서 $S \to E$의 최단 경로 $P$를 구하라.
   - 입력: Graph $G$, Start $S$, End $E$
   - 출력: 경로 $P$

2. **길이 문제**: 그래프 $G$에서 $S \to E$의 최단 경로 길이 $L$을 구하라.
   - 입력: Graph $G$, Start $S$, End $E$
   - 출력: 길이 $L$

3. **Decision 문제**: 그래프 $G$에서 $S \to E$의 최단 경로 길이가 $L$보다 짧은가?
   - 입력: Graph $G$, Start $S$, End $E$, Length $L$
   - 출력: yes / no

**3번(Decision)으로 2번(길이)의 답을 구할 수 있을까?**

Binary Search로 가능하다. $L$의 범위를 최솟값(edge 최소 길이)과 최댓값(모든 edge의 합)으로 잡고, 중간값에 대해 3번 문제를 반복 호출하면 최단 거리 $L$을 구할 수 있다.

**2번(길이)으로 1번(경로)의 답을 구할 수 있을까?**
...
따라서 **3번 Decision Problem만으로도 1번 문제의 답을 구할 수 있다.** 물론 시간·공간적으로 비효율적이지만, 핵심은 "Decision Problem만으로 모든 문제를 해결할 수 있다"는 사실이다.
...
**결론: 풀리지 않는 문제가 압도적으로 많다.**

우리가 풀 수 있는 문제(Solution이 존재하는 문제)는 전체 문제 중 극히 일부에 불과하다.
