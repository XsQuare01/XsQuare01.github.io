...
<li><strong>Class E</strong>: 열거 가능한 언어(Enumerable, RE) — TM이 $x \in L$이면 accept로 정지, $x \notin L$이면 루프 가능</li>
<li><strong>Class co-E</strong>: E의 보집합 언어 — $x \notin L$이면 accept로 정지, $x \in L$이면 루프 가능</li>
...
<p>비결정적 계산(열거 가능성)이란 "답이면 찾을 수 있지만, 답이 아닌 경우 영원히 헤맬 수 있다"는 것이다. 결정 가능한 문제(Class D)는 항상 Yes/No로 끝나지만, 열거 가능한 문제(Class E)는 Yes일 때만 확실히 끝난다. 정지 문제가 대표적인 예다 — 프로그램이 멈추면 "멈춘다"고 답할 수 있지만, 안 멈추는 경우에는 기다리는 것만으로는 알 수 없다.</p>
</div>
...
### 길이 우선 사전순 나열과 결정 가능성

나열하는 순서를 좁히면 열거 가능성에서 결정 가능성을 끌어낼 수 있다. 여기서 쓰는 순서는 **길이 우선 사전순(shortlex)** 이다. 먼저 길이로 줄을 세우고, 길이가 같은 것끼리만 사전순으로 놓는다.

$$
\varepsilon,\ a,\ b,\ aa,\ ab,\ ba,\ bb,\ aaa,\ \ldots
$$

그냥 사전순으로는 안 된다. 사전순에서는 $a$ 다음에 $aa$, $aaa$, $aaaa$가 끝없이 이어지므로 $ab$보다 앞선 문자열이 **무한히 많다.** 어떤 문자열이 나오기를 기다리다 영영 만나지 못할 수 있다. 길이 우선 사전순에서는 각 문자열보다 앞선 것이 유한하므로 이런 일이 생기지 않는다.

> **정리**: L을 길이 우선 사전순으로 열거하는 TM이 존재하면, L∈D이다.

**증명**: $x$가 $L$에 속하는지 결정하려면 열거를 시작한다. $x$보다 앞선 문자열은 유한하므로, 열거는 유한 단계 안에 $x$ 자신이나 $x$보다 뒤인 문자열에 이르거나 아니면 도중에 멈춘다.

$x$가 출력되면 $x \in L$이다. $x$보다 뒤인 문자열이 먼저 출력되면 그때까지 $x$가 나오지 않았으므로 $x \notin L$이다. 열거가 도중에 멈추면 더 나올 것이 없으므로 역시 $x \notin L$이다. 어느 경우든 유한 시간에 답이 정해지고 TM이 정지한다. ∎

길이 우선 사전순이 아닌 임의 순서의 열거라면 이 논리가 성립하지 않는다. "x가 아직 나오지 않았다"는 사실이 "x∉L"을 보장하지 않기 때문이다.
...
그림에서 E와 co-E 바깥에도 언어가 남아 있다. 세어 보면 알 수 있다. TM은 유한한 문자열로 적을 수 있으므로 TM 전체는 가산 무한이고, 열거기를 갖는 언어도 그만큼뿐이라 E는 가산 무한이다. co-E도 마찬가지다. 반면 알파벳 $\Sigma$ 위의 언어 전체는 $2^{\Sigma^*}$이므로 [불가산 무한](/blog/cardinality)이다. 가산 무한 두 개를 합쳐도 불가산을 덮지 못하니, **어느 쪽에도 속하지 않는 언어가 남는다.** 오히려 그런 언어가 압도적으로 많다.
...
이 문제는 결정 불가능($L_a \notin D$)하다. $L_{halt}$에서의 환산(reduction)으로 증명한다.

$L_a$를 판정하는 결정기 $R$이 있다고 하자. 이 $R$로 $L_{halt}$의 결정기를 만들어 보인다. 입력 $\langle M, x \rangle$을 받으면 다음 기계 $M_x$를 소스 코드 수준에서 찍어 낸다.

> $M_x$: 자기가 받은 입력은 무시한다. 테이프에 $x$를 적고 $M$을 흉내 낸다. 흉내 내는 동안에는 `a` 대신 다른 기호를 쓰도록 $M$의 알파벳을 바꿔 둔다. $M$이 정지하면 그때 비로소 `a`를 쓴다.

$x$가 $M_x$ 안에 박혀 있다는 점이 핵심이다. $M_x$는 어떤 입력을 받든 늘 같은 일을 하므로, "어떤 입력에 대해"라는 $L_a$의 조건이 "$M(x)$가 정지하는가" 하나로 줄어든다.

- $M(x)$가 정지하면 $M_x$는 어떤 입력에서든 `a`를 쓰므로 $\langle M_x \rangle \in L_a$
- $M(x)$가 정지하지 않으면 $M_x$는 어떤 입력에서도 `a`를 쓰지 않으므로 $\langle M_x \rangle \notin L_a$

$M_x$를 만드는 일은 $M$의 전이 규칙 앞에 $x$를 적는 부분을 붙이고, 기호를 바꾸고, 정지 상태에서 `a`를 쓰게 고치는 기계적 작업이다. 그러므로 $\langle M, x \rangle$에 대해 $M_x$를 만들어 $R(\langle M_x \rangle)$을 돌리면 $L_{halt}$를 판정하게 된다. $L_{halt} \notin D$이므로 모순이고, 따라서 $L_a \notin D$이다.
...
<div class="callout-title">핵심 정리: $D = E \cap \text{co-E}$</div>
<ul>
<li>$D \subseteq E$: 결정기는 열거기이기도 하다. 항상 정지하는 TM은 accept 조건을 당연히 만족한다.</li>
<li>$E \neq D$: 정지 문제($L_{halt}$)는 E에 속하지만 D에는 속하지 않는다. 자기 참조 모순으로 증명한다.</li>
<li>$D = E \cap \text{co-E}$: $L$과 $\bar{L}$ 모두 열거 가능하면, Dovetailing으로 결정기를 구성할 수 있다.</li>
...
<p><strong>Alice and Bob — 계산 복잡도 클래스 P, NP, PSPACE</strong> — 암호화의 안전성을 계산 복잡도로 정의한다. P, NP, EXP, PSPACE 클래스와 $P \subseteq NP \subseteq PSPACE \subseteq EXP$ 계층 관계를 살펴본다.</p>
</div>
