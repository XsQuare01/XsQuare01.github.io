...
<li><strong>암호학적 함의</strong>: $P \neq \text{NP}$라면 Eve는 도청한 암호문만으로 복호화할 수 없다</li>
</ul>
</div>
...
- **Bob(수신자)**: 암호문 $c$와 증명서 $h$를 가진다. $h$는 증명서 역할을 한다. 검증자 $V$를 실행해 다항식 시간에 복호화한다.
...
<li>$P \neq \text{NP}$ 가정이 성립하면, 증명서(key) 없는 Eve는 암호문을 현실적 시간 안에 풀 수 없다</li>
