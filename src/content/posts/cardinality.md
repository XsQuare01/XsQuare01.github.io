...
이를 길이 관점으로도 볼 수 있다. 유리수를 $q_1,q_2,\dots$처럼 나열한 뒤, 각 $q_n$을 길이 $\varepsilon/2^n$인 작은 구간으로 덮자. 그러면 전체 길이는 $\sum_{n=1}^{\infty}\varepsilon/2^n=\varepsilon$이므로, $\varepsilon$을 임의로 작게 잡으면 유리수 전체를 덮는 총 길이, 즉 확률 측도를 $0$에 가깝게 만들 수 있다. 따라서 유리수를 뽑을 확률은 $0$이다.
