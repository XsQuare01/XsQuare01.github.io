...
> [quick sort](/blog/quicksort)는 최악의 경우 $O(n^2)$까지 느려진다. 만약 pivot으로 항상 중앙값을 고를 수 있다면, 최악에도 $O(n \log n)$이 보장된다. 그런데 중앙값을 찾는 것 자체가 문제다. 이것이 바로 **선택 문제**(selection problem)다. 선택을 $O(n)$에 풀면, quick sort의 최악도 $O(n \log n)$으로 묶인다.
...
[quick sort](/blog/quicksort)의 분할(partition)을 그대로 가져온다. pivot $p$를 하나 잡고 배열을 한 번 훑으면($O(n)$), pivot의 최종 위치 $j$가 정해진다. 이때 pivot의 **등수**는 $j + 1$이다.

- $k = j + 1$이면 $a[j]$가 바로 답이다.
- $k < j + 1$이면 $k$번째는 왼쪽 영역에만 있다 — 오른쪽은 볼 필요가 없다.
- $k > j + 1$이면 $k$번째는 오른쪽 영역에만 있다 — 왼쪽은 볼 필요가 없다.

quick sort가 양쪽 모두를 재귀하는 것과 달리, **quickselect는 한쪽만 재귀**한다. 이것이 핵심이다.

![partition 후 pivot 등수와 k를 비교해 관계없는 쪽은 버리고 한쪽만 재귀하는 quickselect 구조](/images/selection/quickselect-one-side.svg)
...
**최선**은 pivot이 매번 중앙값에 가까울 때다. 분할 후 배열이 절반씩 줄어드는 등비수열이 된다.
...
**최악**은 pivot이 매번 끝값(최솟값 또는 최댓값)일 때다. 분할 후 한쪽이 $n-1$개로 고작 하나씩만 줄어든다.
...
![정렬된 수직선 위 가운데 40% 띠(approximate median 영역)와 좌우 30%, 최악 시 재귀 크기 0.7n 표시](/images/selection/approximate-median.svg)

먼저 이런 pivot이 **quick sort**에 어떤 효과를 주는지 보자. Approximate median을 pivot으로 쓰면 분할 후 두 부분 배열의 크기는 각각 최소 $0.3n$, 최대 $0.7n$이다(둘의 합은 $n$). 핵심은 **양쪽 모두 $n$의 상수 배수**라는 점이다. 가장 큰 가지를 따라가도 $0.7^d n \le 1$이 되는 깊이가 $d = O(\log n)$이므로 재귀 트리의 깊이가 $O(\log n)$으로 묶이고, 각 레벨의 작업량 합이 $n$ 이하이므로 quick sort의 비용 합은 $O(n \log n)$이다. 점화식으로 쓰면:

$$
Q(n) = n + Q(0.3n) + Q(0.7n)
$$

이를 재귀 트리로 그려 보면, 각 레벨의 작업량 합은 항상 $n$ 이하이고 트리 깊이는 $O(\log n)$이므로 전체 합은 $O(n \log n)$이다. 즉, pivot이 가운데 40% 안에만 들어오면 quick sort는 항상 $O(n \log n)$을 유지한다.

다만 이 $Q(n)$은 **quick sort**의 비용이지 선택의 비용이 아니다. 선택 알고리즘은 양쪽을 모두 푸는 것이 아니라 **한쪽만 재귀**하므로, 비용은 다음 섹션에서 $S(n)$으로 따로 세운다. 우선 남은 문제는 그런 approximate median pivot을 어떻게 **빠르게** 찾느냐다.
...
결론: $X'$는 전체에서 대략 30%와 70% 사이에 놓인다. 즉 **가운데 40%** 안의 값이다. 따라서 $X'$를 pivot으로 분할하면 양쪽에 각각 $0.3n$쯤이 들어가, 어느 쪽으로 재귀하든 크기가 대략 $0.7n$으로 묶인다. Approximate median 조건을 만족한다.

<div class="callout">
<div class="callout-title">정확히는 $0.3n$이 아니다</div>
...
2. **"중앙값의 절반"에 올림이 붙는다.** 그룹은 $\lceil n/5 \rceil$개이고 그중 중앙값이 $X'$ 이상인 것은 $\left\lceil \frac{1}{2}\lceil n/5 \rceil \right\rceil$개다. $n/10$과 정확히 같지 않다.
3. **$X'$가 속한 그룹은 빼야 한다.** 그러지 않으면 $X'$ 자신을 양쪽에서 한 번씩 세게 된다.

5개 미만인 그룹과 $X'$의 그룹, 이 둘을 제외하고 다시 세면 $X'$ 이상인 원소의 하한은

$$
3\left(\left\lceil \frac{1}{2}\left\lceil \frac{n}{5} \right\rceil \right\rceil - 2\right) \ge \frac{3n}{10} - 6
$$

이다. 대칭으로 $X'$ 이하도 같다. 따라서 재귀하는 쪽의 크기는 $0.7n$이 아니라 $\frac{7n}{10} + 6$ 이하다.

작은 $n$에서는 실제로 깨진다. $n = 6$일 때 그룹이 $\{2,3,4,5,6\}$과 $\{1\}$로 나뉘면 그룹 중앙값은 $4$와 $1$이고, 이 둘의 중앙값 $X' = 1$이다. $X'$ 이하인 원소는 자기 자신 하나뿐이라 $0.3 \times 6 = 1.8$에 미치지 못한다. $n = 7, 8, 9$에서도 같은 일이 일어난다. 원인은 언제나 5개를 채우지 못한 그룹이다.

**결론은 바뀌지 않는다.** 선형성을 만드는 것은 $\frac{1}{5} + \frac{7}{10} = \frac{9}{10} < 1$이라는 부등호인데, $+6$과 올림은 상수라 이 부등호를 건드리지 못한다. 어떤 상수 $n_0$ 아래의 입력은 그냥 정렬해 답을 내면 되므로, 아래 분석은 이 상수항을 흘려보낸 이상화된 형태로 진행한다.
...
Approximate median을 pivot으로 쓰면, 분할 후 재귀하는 쪽 배열 크기가 최대 $\frac{7n}{10} + 6$이다. 올림과 상수항까지 적으면 점화식은 이렇다.

$$
S(n) \le S\!\left(\left\lceil \frac{n}{5} \right\rceil\right) + S\!\left(\frac{7n}{10} + 6\right) + O(n)
$$

여기서 $+6$과 올림은 두 재귀 크기의 비율 합 $\frac{1}{5} + \frac{7}{10} = \frac{9}{10}$을 바꾸지 못한다. 아래 분석은 상수항을 흘려보낸 이상화된 형태로 진행한다.
...
- **Quickselect**는 quick sort의 분할을 재활용해 한쪽으로만 재귀한다. 평균 $O(n)$이지만 최악은 $O(n^2)$.
...
- 양쪽 $0.3n$ 보장은 **정확한 값이 아니라 근사**다. 5개를 못 채운 그룹과 올림 때문에 실제 하한은 $\frac{3n}{10} - 6$이고, $n = 6, 7, 8, 9$에서는 $0.3n$이 실제로 깨진다. 선형성을 만드는 것은 비율 합 $\frac{9}{10} < 1$이라 상수항은 결론을 바꾸지 않는다.
...
<p>Quickselect의 평균이 정말 $O(n)$인지 기댓값으로 유도하려면 <a href="/blog/selection-average">quickselect 평균 O(n) 유도</a>를 보자. 왜 3개가 아닌 5개로 묶어야 하는지는 <a href="/blog/selection-why-five">왜 5개로 나누는가</a>에서 다룬다. 이 글의 바탕이 된 분할 알고리즘은 <a href="/blog/quicksort">quick sort</a>에, 점화식을 푸는 일반 도구는 <a href="/blog/divide-and-conquer">분할 정복</a>에서 다룬다.</p>
</div>
