...
**정확한 중앙값이 아니어도 된다.** pivot이 전체의 **가운데 40%**, 즉 전체 중 25번째~75번째 백분위 사이의 값이면 충분하다. 이런 pivot을 **approximate median**이라 부른다.

Approximate median을 쓰면 분할 후 한쪽 배열의 크기가 최대 $\frac{3}{4}n$까지만 커진다. (한쪽이 최대 75%.) 이 경우의 점화식은:

$$
Q(n) = n + Q\!\left(\frac{1}{4}n\right) + Q\!\left(\frac{3}{4}n\right)
$$

이를 재귀 트리로 그려 보면, 각 레벨의 작업량 합은 항상 $n$ 이하이고 트리 깊이는 $O(\log n)$이므로 전체 합은 $O(n \log n)$이다 — 퀵 정렬에서와 같은 논리다.

즉, pivot이 가운데 40% 안에만 들어오면 퀵 정렬은 항상 $O(n \log n)$을 유지한다. 문제는 그런 pivot을 어떻게 **빠르게** 찾느냐다.
...
$X'$는 $\lceil n/5 \rceil$개의 그룹 중앙값들의 중앙값이므로, **그룹 중앙값 중 절반($\approx n/10$개)이 $X'$보다 작다.**

각 그룹은 5개이고, 그룹 중앙값이 $X'$보다 작은 그룹에서는 중앙값 포함 아래 3개($X_i$ 이하 원소)가 $X'$보다 작다. 따라서 $X'$보다 작음이 보장되는 원소 수는:

$$
3 \times \frac{n}{10} = 0.3n
$$

대칭 논리로 $X'$보다 크게 보장되는 원소도 $0.3n$개다.

결론: $X'$는 전체에서 최소 30%, 최대 70% 위치에 놓인다 — 즉 **가운데 40%** 안의 값이다. Approximate median 조건을 정확히 만족한다.
...
$S(n) = an + b$로 놓고(선형이라 가정), 점화식에 대입한다.

$$
an + b = 2n + a(0.2n) + b + a(0.7n) + b
$$
...
$$
a = 2 + 0.9a \implies 0.1a = 2 \implies a = 20
$$
...
따라서 $S(n) = 20n$, 즉 $S(n) = O(n)$이다.

$0.2 + 0.7 = 0.9 < 1$이기 때문에 두 재귀의 크기 합이 $n$보다 작아 선형 해가 존재한다. 이것이 median of medians 알고리즘이 최악에도 $O(n)$인 이유다.
...
- **Median of medians**는 5개씩 묶어 approximate median을 $O(n)$에 찾는다. 점화식 $S(n) = 2n + S(0.2n) + S(0.7n)$을 $S(n) = an$ 가정으로 풀면 $a = 20$, 즉 최악에도 $O(n)$.
