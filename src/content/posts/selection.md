...
먼저 이런 pivot이 **퀵 정렬**에 어떤 효과를 주는지 보자. Approximate median을 쓰면 분할 후 한쪽 배열의 크기가 최대 $\frac{3}{4}n$까지만 커진다. (한쪽이 최대 70%대지만, 여유 있게 75%로 잡아도 결론은 같다.) 퀵 정렬은 양쪽을 모두 정렬하므로 정렬 비용의 점화식은:
