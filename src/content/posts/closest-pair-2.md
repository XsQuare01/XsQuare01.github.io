...
밴드를 별도 배열에 담는 것은 읽기 쉬운 대신 **호출마다 배열 하나를 새로 할당한다.** 강의 노트는 이 대가를 지적하며 별도 배열 없이 밴드 밖 점을 건너뛰는 구현을 택한다. 복잡도 차수는 같고 상수 인자만 달라지므로, 아래 코드는 흐름이 드러나는 쪽을 골랐다.
...
<div class="thm">
<div class="thm-head">보조정리 1<span class="en">combine 비용</span></div>

재귀가 y정렬 결과를 물려주면, 한 레벨의 combine은 $O(n)$이다.

</div>
<div class="proof">

<span class="proof-lead">증명.</span> combine의 세 단계 비용을 더한다. 두 y정렬 절반을 합치는 **merge**가 $O(n)$, 이미 y정렬된 배열을 한 번 훑는 **밴드 추리기**가 $O(n)$, 각 점에서 **다음 7개 비교**가 $7n = O(n)$이다. 정렬이 사라져 세 항 모두 $O(n)$이므로 combine은 $O(n)$이다. <span class="qed">∎</span>
...
y정렬 결과를 재귀로 물려주는 개선판의 전체 시간 복잡도는 $O(n \log n)$이다.

</div>
<div class="proof">

<span class="proof-lead">증명.</span> 크기 $n$ 문제를 절반 두 개로 나누고, 보조정리 1에 의해 한 레벨의 combine이 $O(n)$이므로 점화식은 $T(n) = 2\,T\!\left(\tfrac{n}{2}\right) + O(n)$이다. 이는 merge sort와 같은 점화식이라 대입법으로 $T(n) = O(n \log n)$이다(유도는 [분할 정복](/blog/divide-and-conquer)). 맨 처음의 x정렬 $O(n \log n)$은 한 번뿐이라 지배항을 바꾸지 않는다. <span class="qed">∎</span>

</div>

이 결과를 더 줄일 수 있는지는 하한을 봐야 안다. [1편](/blog/closest-pair-1)에서 1차원 가장 가까운 쌍이 element uniqueness를 풀어 주고, element uniqueness는 대수적 결정 트리 모형에서 $\Omega(n \log n)$이 하한임을 짚었다. 2차원은 1차원을 특수한 경우(모든 점을 한 직선에 두기)로 포함하므로 같은 하한을 물려받는다. 따라서 **그 모형 안에서** $O(n \log n)$은 최적이다. 모형을 벗어난 계산(예: 해싱이나 정수 연산의 비트 조작을 허용하는 모형)에서는 이 하한이 그대로 적용되지 않는다.
...
- 이는 **대수적 결정 트리 모형**의 하한 $\Omega(n \log n)$과 같은 차수로, 그 모형 안에서 최적이다.
...
분할 정복이 아닌 다른 시선으로도 같은 $O(n \log n)$에 닿을 수 있다. 점들을 x좌표 순으로 훑으며 "현재 위치에서 폭 $D$ 안의 점들"만 후보로 관리하는 **Plane Sweeping**(평면 훑기) 관점을, 균형 이진 탐색 트리와 함께 [3편](/blog/closest-pair-3)에서 다룬다.
