...
<span class="proof-lead">증명.</span> 크기 $n$ 문제를 절반 두 개로 나누고, 보조정리 1에 의해 한 레벨의 combine이 $O(n)$이므로 점화식은 $T(n) = 2\,T\!\left(\tfrac{n}{2}\right) + O(n)$이다. 이는 merge sort와 같은 점화식이라 대입법으로 $T(n) = O(n \log n)$이다(유도는 [분할 정복](/blog/divide-and-conquer)). 맨 처음의 x정렬 $O(n \log n)$은 한 번뿐이라 지배항을 바꾸지 않는다. 정렬의 하한 $\Omega(n \log n)$과 같은 차수이므로 이 문제는 이론적 하한에 도달한다. <span class="qed">∎</span>
