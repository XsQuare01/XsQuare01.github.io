...
$P$의 x좌표를 $x_P$라 하자. x좌표 차이가 $D$를 넘는 점은 — 그것만으로 이미 거리가 $D$ 이상이므로 — $P$의 후보가 될 수 없다. 우리는 x가 작은 순으로 훑고 있으니, $P$보다 **왼쪽**에 있으면서 x좌표가 $x_P - D$ 이상인 점들만 후보다.
...
x로 폭 $D$, y로 폭 $2D$ — 후보는 결국 $D \times 2D$짜리 직사각형 안에 있다. 그런데 활성 집합 안의 어떤 두 점도 서로 $D$ 이상 떨어져 있다(그렇지 않았다면 `best`가 더 작았을 것이다). **"서로 $D$ 이상 떨어진 점은 $D \times 2D$ 직사각형에 몇 개 못 들어간다"** — 이는 1편에서 밴드를 셀 때 쓴 것과 **똑같은 칸 세기 논거**다. 직사각형을 한 변 $D/2$인 칸으로 쪼개면 칸당 최대 1점, 그래서 후보는 $n$과 무관한 **상수 개**다.
...
모든 항이 $O(n \log n)$이므로 전체도 $O(n \log n)$이다. 2편의 분할 정복과 **같은 차수**, 즉 정렬의 하한 $\Omega(n \log n)$에 닿는 최적이다. 서로 다른 두 길이 같은 곳에서 만난 셈이다.
