...
[퀵 정렬 평균 분석](/blog/quicksort)부터 떠올려 보자. pivot의 등수를 $k$라 하면, 퀵소트는 왼쪽 $k-1$개와 오른쪽 $n-k$개 **양쪽 모두**를 재귀해야 한다. 그래서 점화식에 $E(k-1) + E(n-k)$가 더해진다. 두 비용을 **합산**하기 때문에 결국 $\Theta(n \log n)$이 나온다.
