...
description: "알고리즘의 정의와 올바름(Correctness)을 살펴보고, 최댓값 찾기와 이진 탐색 예시로 알고리즘을 증명한다. 정지 문제(Halting Problem)로 알고리즘의 근본 한계를 확인하고, RAM 모델과 점근적 표기법(Big-O, Θ, Ω)으로 효율을 분석한다."
...
- **종료:** 루프가 $k = n-1$에서 끝나면, 불변식에 의해 `max_val`은 `arr[0..n-1]`의 최댓값.
...
어느 경우에도 모순이 발생한다. 따라서 `halts`는 존재할 수 없다.
...
| $O(2^n)$ | 약 $1.3 \times 10^6$ 년 | — | — | — |

$n = 10^9$인 데이터를 $O(n^2)$ 알고리즘으로 처리하면 **31년**이 걸린다. 같은 문제를 $O(n \log n)$으로 처리하면 **30초**다. 이 비교는 입력 크기가 커질수록 알고리즘의 성장률 차이가 실행 시간 차이로 직접 이어진다는 점을 보여준다.
