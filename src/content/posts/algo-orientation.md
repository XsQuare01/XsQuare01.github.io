...
알고리즘을 이야기할 때는 **문제(problem)**, **입력 인스턴스(instance)**, **출력 조건**을 함께 정해 두면 이후 올바름을 따지기 쉽다. 예를 들어 "배열에서 최댓값 찾기" 문제는 다음과 같이 명세할 수 있다.

- **문제:** 정수 배열에서 가장 큰 원소를 구한다.
- **입력 인스턴스:** 비어 있지 않은 정수 배열 `arr`.
- **출력 조건:** `arr`의 모든 원소보다 작지 않은, `arr`에 속한 값 하나를 반환한다.
...
올바름은 항상 **전제 조건(precondition)** 위에서 말한다는 점에 주의하자. 입력이 전제 조건을 만족할 때, 알고리즘이 종료한 뒤 **사후 조건(postcondition)** 이 성립하면 그 알고리즘은 올바르다. 예를 들어 뒤에서 다룰 이진 탐색은 "배열이 정렬되어 있다"는 전제 조건이 있고, 그 전제를 만족하는 입력에 대해서만 올바름을 보장한다.
...
재귀 알고리즘 분석에서는 강한 귀납법이 더 자연스럽다. 하위 문제 하나가 아니라 **여러 개의 더 작은 하위 문제**에 의존할 때 특히 그렇다. 예를 들어 피보나치 $F(n) = F(n-1) + F(n-2)$는 바로 이전 값 하나가 아니라 두 개의 하위 결과가 모두 올바르다는 가정이 필요하고, merge sort의 올바름 증명도 두 절반에 대한 가정을 동시에 사용한다. (반면 팩토리얼처럼 바로 앞 값 하나에만 의존하는 경우는 약한 귀납법으로 충분하다.)
...
**find_max에 대한 루프 불변식 증명:** 다음과 같이 인덱스 $k$를 1부터 $n-1$까지 옮기며 검사한다고 하자.
...
> **불변식 I:** 인덱스 $k$에 대한 반복을 마친 직후, `max_val`은 `arr[0..k]`의 최댓값이다.

- **초기화:** 첫 반복($k = 1$) 직전에는 `max_val = arr[0]`이므로 `arr[0..0]`의 최댓값 = `arr[0]`. 성립.
- **유지:** $k$에 대한 반복 직전에 `max_val`이 `arr[0..k-1]`의 최댓값이라고 하자. 이 반복에서 `arr[k]`를 읽어 더 크면 갱신하므로, 반복 직후 `max_val`은 `arr[0..k]`의 최댓값이다. 성립.
- **종료:** 마지막 인덱스 $k = n-1$에 대한 반복까지 마치면, 불변식에 의해 `max_val`은 `arr[0..n-1]`, 즉 배열 전체의 최댓값이다.
...
앨런 튜링(Alan Turing, 1936)은 이 문제가 **결정 불가능(Undecidable)** 하다는 것을 증명했다. 즉, **모든 프로그램과 모든 입력에 대해 종료 여부를 항상 올바르게 판정하는 범용 알고리즘은 존재하지 않는다.** (특정 프로그램 하나의 종료 여부를 사람이 따로 분석하는 것까지 불가능하다는 뜻은 아니다.)
...
이 증명의 구조는 칸토어(Cantor)의 대각선 논법과 같으며, [기수(Cardinality)](/blog/cardinality) 포스트에서 다룬 "실수가 자연수보다 많다"는 증명과 같은 대각선화 아이디어를 사용한다.

<div class="callout callout-simple">
<div class="callout-title">정지 문제가 중요한 이유</div>
<p>정지 문제의 결정 불가능성은 소프트웨어 검증에 직접적인 한계를 부여한다. "이 프로그램이 항상 종료하는가?"와 같은 비자명한 동작 성질을 모든 프로그램에 대해 완전히 자동으로 판정하는 범용 알고리즘은 존재하지 않는다 (이는 라이스 정리로 일반화된다). 정적 분석 도구들이 보수적 근사(conservative approximation)를 사용하는 이유가 바로 여기에 있다.</p>
</div>
...
컴퓨터가 초당 $10^9$번 연산을 수행한다고, 즉 **기본 연산 한 단계가 정확히 1 ns**라고 단순하게 가정하고 입력 크기에 따른 실행 시간을 비교해 보자. 로그의 밑은 2로, 1년은 365.25일로 잡아 환산했다.
...
| $O(n^2)$ | 10 μs | 100 ms | 17 min | 약 32 년 |
| $O(2^n)$ | 약 $4 \times 10^{13}$ 년 | — | — | — |

$n = 10^9$인 데이터를 $O(n^2)$ 알고리즘으로 처리하면 **약 32년**이 걸린다. 같은 문제를 $O(n \log n)$으로 처리하면 **30초**다. 이 비교는 입력 크기가 커질수록 알고리즘의 성장률 차이가 실행 시간 차이로 직접 이어진다는 점을 보여준다.

물론 이 표는 한 단계를 1 ns로 본 단순화된 모델이며, 실제 값은 상수 계수와 한 연산의 실제 비용에 따라 달라진다. 특히 로그 시간 값은 로그의 밑에 따라서도 바뀌므로(상수 배 차이) 절대 수치보다 **성장률의 차이**에 주목하는 것이 핵심이다.
...
<div class="callout callout-simple">
<div class="callout-title">표기법과 분석 기준은 별개다</div>
<p>$O$, $\Omega$, $\Theta$는 <strong>함수의 성장률을 비교하는 표기법</strong>일 뿐, 그 자체가 "최악의 경우"를 뜻하지는 않는다. 최선·최악·평균 중 무엇을 분석하느냐는 별도의 선택이다. 예를 들어 "quick sort의 <em>최선</em> 시간은 $O(n \log n)$"처럼, 최악이 아닌 경우에도 Big-O를 쓸 수 있다. 이 글의 여러 예시가 최악을 기준으로 삼을 뿐이다.</p>
</div>
...
| 복잡도 | 이름 | 대표 알고리즘 |
|---|---|---|
| $O(1)$ | 상수 | 배열 인덱스 접근, 해시 테이블 조회(평균) |
| $O(\log n)$ | 로그 | 이진 탐색, 균형 이진 트리 |
| $O(\sqrt{n})$ | 제곱근 | 소수 판정(시험 나눗셈), 약수 열거 |
| $O(n)$ | 선형 | 순차 탐색, 배열 순회 |
| $O(n \log n)$ | 선형 로그 | merge sort, heap sort, quick sort(평균) |
| $O(n^2)$ | 이차 | insertion sort, 버블 정렬, selection sort |
| $O(n^3)$ | 삼차 | 행렬 곱셈(나이브), Floyd-Warshall |
| $O(2^n)$ | 지수 | 부분집합 완전 탐색, 피보나치(단순 재귀) |
| $O(n!)$ | 팩토리얼 | 순열 완전 탐색, TSP 브루트포스 |

해시 테이블 조회의 $O(1)$은 평균 기준이다. 충돌이 한곳에 몰리면 최악 비용은 구현과 충돌 처리 방식에 따라 $O(n)$까지 늘어난다.

---

## 예제 1: 배열에서 최댓값 찾기

**전제 조건:** `arr`는 비어 있지 않다 (`max_val = arr[0]`에서 첫 원소를 읽기 때문이다).
...
위 코드는 설명을 위한 Python식 의사코드다. `arr[1:]` 슬라이싱은 실제로는 새 리스트를 만들 수 있지만, 여기서는 RAM 모델에서 한 원소씩 순회하는 것으로 보고 슬라이싱 비용은 제외한다.
...
위 그림에서 `left`, `right`, `mid`는 다음과 같이 움직인다. 처음에 `left=0`, `right=7`이면 `mid=3`(값 11)이고, `11 < 14`이므로 오른쪽 절반으로 좁혀 `left=4`가 된다. 다음에는 `mid=5`(값 17)이고, `17 > 14`이므로 왼쪽으로 좁혀 `right=4`가 된다. 마지막에 `mid=4`(값 14)에서 목표를 찾는다. 매 단계마다 탐색 범위가 절반으로 줄어드는 것을 볼 수 있다.
...
<li>점근적 표기법: Big-O는 <strong>상한</strong>, Big-Ω는 <strong>하한</strong>, Big-Θ는 <strong>긴밀한 경계</strong>다(상한과 하한이 같은 성장률). 상수 계수와 낮은 차수 항은 무시한다.</li>
