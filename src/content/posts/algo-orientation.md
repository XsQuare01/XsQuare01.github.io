...
재귀 알고리즘 분석에서는 강한 귀납법이 더 자연스럽다. 하위 문제 하나가 아니라 **여러 개의 더 작은 하위 문제**에 의존할 때 특히 그렇다. 예를 들어 피보나치 $F(n) = F(n-1) + F(n-2)$는 바로 이전 값 하나가 아니라 두 개의 하위 결과가 모두 올바르다는 가정이 필요하고, 병합 정렬의 올바름 증명도 두 절반에 대한 가정을 동시에 사용한다. (반면 팩토리얼처럼 바로 앞 값 하나에만 의존하는 경우는 약한 귀납법으로 충분하다.)
...
<p>$O$, $\Omega$, $\Theta$는 <strong>함수의 성장률을 비교하는 표기법</strong>일 뿐, 그 자체가 "최악의 경우"를 뜻하지는 않는다. 최선·최악·평균 중 무엇을 분석하느냐는 별도의 선택이다. 예를 들어 "퀵 정렬의 <em>최선</em> 시간은 $O(n \log n)$"처럼, 최악이 아닌 경우에도 Big-O를 쓸 수 있다. 이 글의 여러 예시가 최악을 기준으로 삼을 뿐이다.</p>
</div>
