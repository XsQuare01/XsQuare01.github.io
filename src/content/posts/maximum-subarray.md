...
[동적 계획법 ②](/blog/dp-2)는 "마지막 날에 일하는가"라는 결정 하나로 점화식을 세웠다. 여기서도 결정은 하나, "이 자리를 포함하는가"다. 결정이 깔끔하게 두 갈래로 갈리는 것은 부분 문제를 "$a_i$ 에서 끝나는"으로 잡았기 때문이다. 부분 문제를 "$a_1 \dots a_i$ 안에서의 답"으로 잡으면 이렇게 되지 않는다. 앞의 답이 어디서 끝났는지 모르면 $a_{i+1}$ 을 이어 붙일 수 있는지 판단할 수 없다.
