...
부분 문제는 "뒷조각"(suffix)으로 잡는다. $a_1 \dots a_i$ 의 뒷조각이란 그 앞부분을 어디선가 잘라내고 남은 $a_s, \dots, a_i$ 를 말한다. 아무것도 남기지 않은 빈 뒷조각도 여기 넣는다. 빈 것을 빼면 뒷조각은 곧 $a_i$ 에서 끝나는 구간이고, 빈 것까지 넣어야 위에서 못박은 정의와 맞아떨어진다.
