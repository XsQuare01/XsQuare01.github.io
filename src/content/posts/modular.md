...
> 모듈러 산술은 정수를 $m$으로 나눈 나머지가 같은 것끼리 묶어 하나의 원소로 다루는 구조다. 이렇게 묶은 합동류 위에 덧셈과 곱셈을 정의하면 유한한 집합에서 대수 연산이 돌아가고, $m$이 소수일 때는 $0$이 아닌 원소마다 곱셈 역원이 생긴다. RSA, ElGamal, Diffie-Hellman이 모두 이 구조 위에서 계산한다.
...
여기서 $a$와 $b$는 **양의 정수**라고 둔다. 음수를 섞으면 아래 등식이 $\gcd(a,b) \cdot \text{lcm}(a,b) = |ab|$ 꼴로 바뀌기 때문이다.
...
$\mathbb{Z}_m$에서 덧셈과 뺄셈을 이야기하려면 두 가지를 따로 확인해야 한다. 이름이 비슷해 자주 섞이지만 다른 성질이다.

- **잘 정의됨(well-defined)**: 합동류에서 어떤 대표원을 꺼내 계산해도 결과가 같은 합동류에 떨어진다. $a \equiv a'$, $b \equiv b'$이면 $a + b \equiv a' + b'$이므로 성립한다. 이것이 없으면 「합동류끼리 더한다」는 말 자체가 뜻을 갖지 못한다.
- **닫힘(closure)**: 연산 결과가 다시 $\mathbb{Z}_m$의 원소다. 나머지는 언제나 $0$부터 $m-1$ 사이이므로 성립한다.

두 성질이 갖춰지면 덧셈에 항등원 $0$과 역원이 있다. 원소 $a$의 덧셈 역원은 $-a \equiv m - a \pmod{m}$이다.
...
이 경우 $\mathbb{Z}_m$은 **체(Field)** 가 된다. 덧셈·뺄셈·곱셈이 모든 원소에서 정의되고, 나눗셈은 **$0$이 아닌 원소로만** 정의된다. 체에서도 $0$으로 나누는 연산은 없다. 「사칙연산이 모두 된다」는 말은 이 단서 아래에서 읽어야 한다.
...
$\prod a_i \in \mathbb{Z}_m^*$이고 $\mathbb{Z}_m^*$의 원소는 모두 곱셈 역원을 가지므로, **양변에 $\left(\prod a_i\right)^{-1}$을 곱하면** 좌우의 $\prod a_i$가 소거되어
