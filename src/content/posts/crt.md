...
**속도 향상 이유.** 비용을 세려면 모형을 먼저 정해야 한다. 모듈러스가 $\ell$비트일 때 교과서 곱셈으로 모듈러 곱셈 한 번이 $O(\ell^2)$이고, 제곱·곱셈 방식은 지수의 비트 수만큼 곱셈을 돌린다. 지수도 $\ell$비트면 지수 연산 한 번은 $O(\ell^3)$이다.

RSA-CRT에서는 **두 가지가 함께 반으로 줄어든다.** 모듈러스가 $n$에서 $p$, $q$로 바뀌어 $\ell/2$비트가 되고, 지수도 $d$에서 $d_p = d mod (p-1)$로 바뀌어 $\ell/2$비트가 된다. 그래서 한 번의 비용이

$$
\left(rac{\ell}{2}ight)^3 = rac{\ell^3}{8}
$$

로 떨어진다. 이것을 두 번 하므로 합이 $\ell^3/4$, 곧 원래의 $1/4$이다. **약 4배 가속**은 여기서 나온다.

지수가 짧아지는 부분을 빼먹으면 각각 $1/4$에 두 번이라 $1/2$, 곧 2배밖에 되지 않는다. 두 몫이 함께 줄어드는 것이 핵심이다.

CRT로 다시 합치는 비용은 $O(\ell^2)$ 수준이라 $\ell^3$ 앞에서 묻힌다. 다만 이 4배는 교과서 곱셈을 전제로 한 값이다. 곱셈에 더 빠른 알고리즘을 쓰면 지수가 달라져 배수도 달라진다.
...
<li>RSA-CRT는 $n = pq$ 구조를 활용해 복호화 속도를 약 4배 향상시킨다. 모듈러스와 지수가 함께 절반이 되어 $\ell^3$ 비용이 $1/8$로 줄고, 그것을 두 번 하므로 $1/4$이다. 교과서 곱셈을 전제로 한 값이다.</li>
</ul>
</div>
