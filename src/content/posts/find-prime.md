...
Find Prime:
  1. 임의의 홀수 n을 선택한다.
  2. n이 Carmichael 수인지 확인한다.
     → 맞으면 1로 돌아간다.
  3. 1 < a < n이고 gcd(a, n) = 1인 임의의 a를 선택한다.
     → gcd(a, n) ≠ 1이면 n은 합성수 → 1로 돌아간다.
  4. a^(n-1) ≡ 1 (mod n)인지 확인한다.
     → 1이 아니면 a는 Witness → n은 합성수 → 1로 돌아간다.
     → 1이면 3으로 돌아가 다른 a로 반복한다.
  k번의 테스트를 모두 통과하면 n을 소수로 판정한다.
```
