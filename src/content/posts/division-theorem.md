...
정수 $a, b, c$에 대해, $b = ca$가 성립할 때 (나머지가 0일 때) $a$가 $b$를 **나눈다**고 하며, 다음과 같이 표기한다.
...
$b$에서 $a$를 반복해서 빼면 반드시 음수가 된다. $a > 0$이므로 $k$를 충분히 크게 잡으면 $ka$가 $b$를 넘어서기 때문이다(아르키메데스 성질). 그런 $k$ 가운데 가장 작은 것, 곧 $b - ka < 0$이 되는 최솟값 $k$를 잡으면:
