...
볼록 껍질 알고리즘은 꼭짓점을 아무 순서로나 내놓지 않는다. Graham Scan이 그랬듯 **다각형의 둘레를 도는 순서**로 내놓는다. 그런데 원 위에서 둘레를 도는 순서는 곧 **각도가 커지는 순서**이고, 각도는 1단계에서 원래 숫자에 비례하도록 만들었다. 정리하면,
