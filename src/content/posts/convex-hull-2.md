...
**서로 다른** 양수 $v_1, \cdots, v_N$을 정렬한다고 하자. 값이 겹치면 아래 변환이 두 숫자를 같은 점으로 보내 버리므로 서로 다른 경우로 한정하는데, 비교 정렬의 하한은 서로 다른 키에 대해서도 성립하므로 논증의 힘은 그대로다. 최댓값 $M$을 찾은 뒤($O(N)$), 각 숫자를 **원 위의 점**으로 바꾼다.
