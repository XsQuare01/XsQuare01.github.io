...
**안쪽 추가라면 껍질은 그대로다.** 판정은 두 사슬로 나눈 덕에 쉽다. $X$의 x좌표 위에 놓인 변을 upper hull에서 이진 탐색으로 찾아 $X$가 그 변보다 아래인지 CCW로 보고, lower hull에서도 같은 것을 본다. upper hull 아래이면서 lower hull 위이면 $X$는 내부이고, 아무것도 할 일이 없다.
